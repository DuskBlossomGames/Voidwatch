...
public class SmartBoidHandler : MonoBehaviour
{
    ...
    void Start()
    {
        _colmask = LayerMask.GetMask(new string[] { "Self-Colliding Enemies", "Enemies", "Player", "Scene Objects", "Goo", "Hazards", "Player Bullets"});
    ...
    void Update()
    {
        ...
        for (int i = 0; i < numRays; i++)
        {
            ...
            if (collision.collider != null)
            {
                ...
                Debug.LogFormat("Collided object is {0}", collision.collider.gameObject.name);
            } else
            {
                //var a = new Vector2(Mathf.Cos(Mathf.Deg2Rad * (90 + rayAngle)), Mathf.Sin(Mathf.Deg2Rad * (90 + rayAngle)));
