...
public class ChaserBehavior : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (_driftTimer.IsFinished)
        {
            ...
            var vel = GetComponent<Util.CustomRigidbody2D>().velocity;
            GetComponent<Util.CustomRigidbody2D>().velocity = UtilFuncs.LerpSafe(vel,_movDir * _currSpeed, 10 * Time.deltaTime);
