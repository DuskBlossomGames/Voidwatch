    ...
    public class MiniPlayerController : MonoBehaviour
    {
        ...
        private IEnumerator DoGoTo(Vector3 planetLoc, int planetIdx, string scene)
        {
            ...
            for (var i = 1; i < planetPath.Length; i++)
            {
                ...
                var endPos = target + (current - target).normalized * (i == planetPath.Length-1 ? _orbitRadius/2 : _orbitRadius);
            ...
            {
                for (float t = 0; t < 1; t += Time.fixedDeltaTime / (Vector2.Distance(path[i].Item1, path[i - 1].Item1) / path[i].Item2))
                {
                    yield return new WaitForFixedUpdate();
                    
                    transform.SetLocalPositionAndRotation(
                        Vector2.Lerp(path[i-1].Item1, path[i].Item1, t),
                        Quaternion.Euler(0, 0, -90 + Mathf.Rad2Deg * UtilFuncs.Angle(path[i].Item1 - path[i - 1].Item1)));
                } 
            }
